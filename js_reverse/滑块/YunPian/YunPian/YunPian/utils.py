...
async def encrypt(key, iv, content):
    """
    AES加密
    模式cbc
    填充pkcs7
    ...
    # 处理明文
    content_padding = await pkcs7padding(content)
    # 加密
...
async def decrypt(key, iv, content):
    """
    AES解密
    模式cbc
    去填充pkcs7
    ...
    # 去除填充内容
    result = await pkcs7unpadding(result)
    return result


async def des3encrypt(plain_text: str, key: str, iv: str) -> str:
...
async def des3decrypt(cipher_text: str, key: str, iv: str) -> str:
    des3 = DES3.new(key=key.encode(), mode=DES3.MODE_CBC, iv=iv.encode())
