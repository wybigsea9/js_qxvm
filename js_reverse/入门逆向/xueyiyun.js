...
for (var i = 0,
r = Math.min(n.length, 4), t = 0; t < r; ++t) i <<= 4,
...
u = n.length,
t = u,
i = 0; t > 0; t -= 4, ++i) r.digits[i] = hexToDigit(n.substr(Math.max(t - 4, 0), Math.min(t, 4)));
return r
}
...
for (var i = "",
t = biHighIndex(n); t > -1; --t) i += digitToBytes(n.digits[t]);
return i
}
function digitToBytes(n) {
var i = String.fromCharCode(n & 255),
t;
return n >>>= 8,
...
for (var r = new BigInt,
u, o = biHighIndex(n), s = biHighIndex(t), e, f, i = 0; i <= s; ++i) {
    for (u = 0, f = i, j = 0; j <= o; ++j, ++f) e = r.digits[f] + n.digits[j] * t.digits[i] + u,
...
for (f = h; f > u; --f) {
    ...
    for (r.digits[f - u - 1] = c == l ? maxDigitVal: Math.floor((c * biRadix + y) / l), p = r.digits[f - u - 1] * (l * biRadix + k), w = c * biRadixSquared + (y * biRadix + b); p > w;)--r.digits[f - u - 1],
...
a,
w;
for (s = typeof i == "string" ? i == RSAAPP.NoPadding ? 1 : i == RSAAPP.PKCS1Padding ? 2 : 0 : 0, v = typeof r == "string" && r == RSAAPP.RawEncoding ? 1 : 0, s == 1 ? o > n.chunkSize && (o = n.chunkSize) : s == 2 && o > n.chunkSize - 11 && (o = n.chunkSize - 11), u = 0, e = s == 2 ? o - 1 : n.chunkSize - 1; u < o;) s ? f[e] = t.charCodeAt(u) : f[u] = t.charCodeAt(u),
u++,
e--;
for (s == 1 && (u = 0), e = n.chunkSize - o % n.chunkSize; e > 0;) {
    if (s == 2) {
        for (c = Math.floor(Math.random() * 256); ! c;) c = Math.floor(Math.random() * 256);
        f[u] = c
    } else f[u] = 0;
    u++;
    e--
}
for (s == 2 && (f[o] = 0, f[n.chunkSize - 2] = 2, f[n.chunkSize - 1] = 0), y = f.length, u = 0; u < y; u += n.chunkSize) {
    for (l = new BigInt, e = 0, h = u; h < u + n.chunkSize; ++e) l.digits[e] = f[h++],
    l.digits[e] += f[h++] << 8;
...
for (var e = t.split(" "), f, u, o, i = "", r = 0; r < e.length; ++r) for (o = n.radix == 16 ? biFromHex(e[r]) : biFromString(e[r], n.radix), f = n.barrett.powMod(o, n.d), u = 0; u <= biHighIndex(f); ++u) i += String.fromCharCode(f.digits[u] & 255, f.digits[u] >> 8);
return i.charCodeAt(i.length - 1) == 0 && (i = i.substring(0, i.length - 1)),
i
}
var lookup = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",
dpl10, lr10, hexatrigesimalToChar, hexToChar, highBitMasks, lowBitMasks, RSAAPP; (function(n) {
"use strict";
function t(n) {
...
function c(n, t) {
    for (var f = [], r, u, i = 0, e = n.length; i < e; i++) typeof n != "string" || !n.charAt(i) ? typeof n != "string" && i in n && (r = n[i], u = t(r, i, n), f[i] = u) : (r = n.charAt(i), u = t(r, i, n), f[i] = u);
    return f
}
function l(n) {
    function u(n) {
    ...
    var i, h, c, r, e, o, s, l;
    if (n.length % 4 > 0) throw new Error("Invalid string. Length must be a multiple of 4");
    for (s = n.length, e = n.charAt(s - 2) === "=" ? 2 : n.charAt(s - 1) === "=" ? 1 : 0, o = new f(n.length * 3 / 4 - e), c = e > 0 ? n.length - 4 : n.length, l = 0, i = 0, h = 0; i < c; i += 4, h += 3) r = t(n.charAt(i)) << 18 | t(n.charAt(i + 1)) << 12 | t(n.charAt(i + 2)) << 6 | t(n.charAt(i + 3)),
    u((r & 16711680) >> 16),
    u((r & 65280) >> 8),
    u(r & 255);
    return e === 2 ? (r = t(n.charAt(i)) << 2 | t(n.charAt(i + 1)) >> 4, u(r & 255)) : e === 1 && (r = t(n.charAt(i)) << 10 | t(n.charAt(i + 1)) << 4 | t(n.charAt(i + 2)) >> 2, u(r >> 8 & 255), u(r & 255)),
    o
}
function a(n) {
    function r(n) {
    ...
    Object.prototype.toString.call(n) != "[object Array]" && (n = c(n,
    function(n) {
        return n.charCodeAt(0)
    }));
    for (var f = n.length % 3,
    t = "",
    i, u = 0,
    e = n.length - f; u < e; u += 3) i = (n[u] << 16) + (n[u + 1] << 8) + n[u + 2],
    t += o(i);
    switch (f) {
    case 1:
        i = n[n.length - 1];
...
function Getpass(p,modulus) {
setMaxDigits(262);
var key = new RSAKeyPair('010001', "", modulus, 2048);
var ciphertext = encryptedString(key, p, RSAAPP.PKCS1Padding, RSAAPP.RawEncoding);
Password = base64js.fromByteArray(ciphertext);
