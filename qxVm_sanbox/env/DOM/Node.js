...
; (function () {
    ...
    Node.prototype = {
        appendChild(aChild) {
            debugger;
            // 当向父级添加的时候，offsetHeight 先=style.height
            // 处理高度的检测, 父级的高度=子的高度
            ...
            if (ele_this){
                if (ele_this.parentNode === ele_child){
                    if (lwVm.config.logOpen=== true) lwVm.console_log('[appendChild]', "重复修改父节点执行，报错了，抛出异常，ele_child=", ele_child, "ele_this=", ele_this);
                    throw new Error("Failed to execute 'appendChild' on 'Node': The new child element contains the parent.");
                }
            }
            // 儿子的父亲节点指向
            ele_child.parentNode = ele_this
            // aChild.__proto__[Symbol.toStringTag]
            if (!Node.prototype.isPrototypeOf(this)) { throw new TypeError("Illegal constructor"); };
            if (lwVm.config.logOpen=== true) lwVm.logAdd('Func', 'Node', 'appendChild[aChild和父亲this]', arguments, this);
            if (lwVm.config.logOpen=== true) lwVm.console_log('[appendChild]', "儿子是", aChild, "父亲是", this);
            return aChild;
        },
        get baseURI() {
            if (!Node.prototype.isPrototypeOf(this)) { throw new TypeError("Illegal constructor"); };
